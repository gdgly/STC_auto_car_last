...
void main()
{
   ...
   /********************差比和*****************************/
        AD_LVBO();
        if(MAD<=1) MAD=1;
        Difference = ((3800*(RAD-LAD))/((LAD+MAD+RAD)*MAD))*Dif_Nor_coefficient;
        Difference = range_protect(Difference, -Dif_Nor_coefficient, Dif_Nor_coefficient);
   /**************环岛判断****(In_Roundabout_I>5500)&&********************** */
        if((LAD>I_LAD)&&(MAD>I_MAD)&&(RAD>I_RAD)&&(LADC>I_LADC)&&(RADC>I_RADC)&&(!Out_Roundabout_Flag))//进环岛
            {
        ...
        if(In_Roundabout_Flag&&Roundabout_cnt>=3)               
            ...
            if((In_Out_Roundabout_ture_I<30)&&(In_Out_Roundabout_ture_I>-30))
                { 
                  	Difference = ((3800*(RADC-LADC))/((LADC+MAD+RADC)*MAD))*Dif_Nor_coefficient;  
                  	Fres = 4;
                }
   ...
   /******************环岛后处理***************************/
        ...
        {  
          	...
          	In_Out_Roundabout_I=0;
           	...
           	Difference = ((3800*((RAD-0.8*RADC)-(LAD-0.8*LADC)))/((LAD+MAD+RAD)*MAD))*Dif_Nor_coefficient;  
