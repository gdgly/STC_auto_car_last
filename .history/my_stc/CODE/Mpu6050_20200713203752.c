 ...
 * @LastEditTime : 2020-07-13 20:37:49
 */
#include "Mpu6050.h"
...
/*
 *函数名：Another_Get_Attitude
 *描述：一节互补算法姿态解算（一节互补滤波解算）
 *输出：
 *输入：
 *调用：
*/
#define K1 = 0.04
const float Acc_to_Ang_need = 57.2958; 
const float dt = 0.001;
float Ang_ACC;
void Another_Get_Attitude(void)
{
  static float Last_angle_Y;
  Ang_ACC = Acc_to_Ang_need*(float)(atan2(-ACC.Z, -ACC.X));
  Attitude_Angle.Y = K1 * Ang_ACC + (1-K1) * (Attitude_Angle.Y + GYRO.Y/7510.0 * dt);
  if(Attitude_Angle.Y-Last_angle_Y)
  Last_angle_Y = Attitude_Angle.Y;
