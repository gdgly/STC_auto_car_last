 ...
 * @LastEditTime : 2020-07-14 15:27:50
 */ 
#include "PID.h"
...
int32 PlacePID_Control(PID *sprt, float *PID, int32 NowPiont, int32 SetPoint)
{
    ...
    */
	...
	Actual = Kp * iError+ PID[KD] * (0.8(*iError - sprt->LastError));//只用PD   原式为0.8*iError+0.2*sprt->LastError- sqrt->LastError;		//更新上次误差

//	Actual += sprt->SumError*0.1;不用I所以不需要误差和
	Actual = range_protect_main(Actual, -TURN_lim, TURN_lim);
    sprt->LastError = iError;           // 更新过去的
	return Actual;
}
...
int32 PID_Realize_D(PID *sptr, float *PID, int32 NowData, int32 Point)
{
	...
	else if (sptr->SumError <= -PID[KT])
	{
        ...
        if(Difference*GYRO_Real.Z>0)      //如果陀螺仪转角和电磁差表明的转角相同，则不需要 D 调节
         SetD =0;
