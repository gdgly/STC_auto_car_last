 ...
 * @LastEditTime : 2020-06-29 17:10:33
 */ 
#include "MyDebug.h"
//上位机相关定义
float Variable[16];
...
{   //按键读值
  KEY_UP    = gpio_get(PTA7);
  KEY_DOWN  = gpio_get(PTF2);    
  KEY_LEFT  = gpio_get(PTA6);
  KEY_RIGHT = gpio_get(PTB0);
  KEY_CEL   = gpio_get(PTF3);
  KEY_SURE  = gpio_get(PTE4);
   //确定键
   if(KEY_SURE==0) 
   {
     //去抖
