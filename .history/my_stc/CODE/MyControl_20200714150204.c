 ...
 * @LastEditTime : 2020-07-14 15:02:04
 */
...
void Speed_Measure(void)
{
  ...
  /******* 右电机速度相关控制 ********/
  MOTOR_Speed_Right = ctimer_count_read(CTIM0_P34); //右轮
  ...
  /******* 右电机速度相关控制结束 ********/
  /******* 左电机速度相关控制 ********/
  MOTOR_Speed_Left = ctimer_count_read(CTIM3_P04); //左轮
  ...
  if (Speed_Min <= Speed_Set * 0.3)    // 当前速度越大，转向曲率对应的PID不同
    Fres = 0;
...
void AD_LVBO()
{
  uint8 i, j;
#define FILTER_N 5
  float filter_temp, filter_sum_L = 0, filter_sum_LC = 0, filter_sum_R = 0, filter_sum_RC = 0;
...
void MOTOR_Control(int32 LDuty, int32 RDuty)
{
  ...
  if (LDuty >= 0)
  {
    ...
    pwm_duty(MOTOR3_PWM, 0);                    // 占空比最大60%！！！
    pwm_duty(MOTOR4_PWM, LDuty);                // 占空比最大60%！！！
  }
  else
  {
    LDuty = range_protect(-LDuty, 0, MOTOR_MAX); // 限幅保护
    pwm_duty(MOTOR4_PWM, 0);                     // 占空比最大60%！！！
    pwm_duty(MOTOR3_PWM, -LDuty);                 // 占空比最大60%！！！
  }
  if (RDuty >= 0)
  {
    RDuty = range_protect(RDuty, 0, MOTOR_MAX); // 限幅保护
    pwm_duty(MOTOR2_PWM, 0);                    // 占空比最大60%！！！
    pwm_duty(MOTOR1_PWM, RDuty);                // 占空比最大60%！！！
  }
  else
  {
    RDuty = range_protect(-RDuty, 0, MOTOR_MAX); // 限幅保护
    pwm_duty(MOTOR1_PWM, 0);                     // 占空比最大60%！！！
    pwm_duty(MOTOR2_PWM, -RDuty);                 // 占空比最大60%！！！
  }
}