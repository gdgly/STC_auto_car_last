 ...
 * @LastEditTime : 2020-07-09 16:24:07
 */
...
void MOTOR_Control(int32 LDuty, int32 RDuty)
{
  ...
  if (LDuty >= 0)
  {
    ...
    pwm_duty(MOTOR3_PWM, 0);                    // 占空比最大75%！！！
    pwm_duty(MOTOR4_PWM, LDuty);                // 占空比最大75%！！！
  }
  else
  {
    LDuty = range_protect(-LDuty, 0, MOTOR_MAX); // 限幅保护
    pwm_duty(MOTOR4_PWM, 0);                     // 占空比最大75%！！！
    pwm_duty(MOTOR3_PWM, -LDuty);                 // 占空比最大75%！！！
  }
  if (RDuty >= 0)
  {
    RDuty = range_protect(RDuty, 0, MOTOR_MAX); // 限幅保护
    pwm_duty(MOTOR2_PWM, 0);                    // 占空比最大75%！！！
    pwm_duty(MOTOR1_PWM, RDuty);                // 占空比最大75%！！！
  }
  else
  {
    RDuty = range_protect(-RDuty, 0, MOTOR_MAX); // 限幅保护
    pwm_duty(MOTOR1_PWM, 0);                     // 占空比最大75%！！！
