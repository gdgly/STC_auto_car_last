 ...
 * @LastEdiors   : yan_wen
 * @LastEditTime : 2020-06-21 20:43:31
 */ 
#include "System.h"
uchar System_OK = 0;
float Zero_Angle;
...
/************************ 陀螺仪 初始化 ******************/
  MPU6050_OPEN();
  initDMPMPU6050();			//dmp方式读取的初始化
  if (getDeviceID())
  {	//验证连接是否正常(读取MPU6050的I2C地址)
  ...
  for (i = 0; i < 200; i++)
  {
    ...
    Get_Attitude();	// 姿态解算
    ar_Ang_Vel.Y = PID_Realize(&Angle_PID, Angle, (int32)(Attitude_Angle.Y*100), Zero_Angle*100);
  }
  Run_Flag = 1;
/*********************** 正确角度发车****************************/
  while (!System_OK)
