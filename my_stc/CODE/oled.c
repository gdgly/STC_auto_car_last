 ...
 * @LastEditTime : 2020-06-29 18:21:59
 */ 
#include  "oled.h"
...
void OLED_Fill(uint8 bmp_dat)
{
	...
	for(y=0;y<8;y++)
	{
		...
		OLED_WrCmd(0x01);       //0x00+0~16表示将128列分成16组其地址在某组中的第几列
...
void OLED_Init(void)        
{
  gpio_mode(P2_0, GPO_PP); //DC
  gpio_mode(P2_3, GPO_PP); //D1
  gpio_mode(P2_5, GPO_PP); //D0
  gpio_mode(P2_1, GPO_PP); //RST
	gpio_mode(P2_0, GPO_PP); //CS
  RST = 0;
  DC = 0;
  D0 = 0;
  D1 = 0;
...
//功能描述：将一个float型数转换成2位整数部分带4位小数和符号的数据并进行显示
 void OLED_PrintValueF(unsigned char x, unsigned char y, float dat, unsigned char num)
 {
	...
	int Integer_Part	 ;
        ...
        if(dat>0)       
        dat=dat+0.00001;
  	Integer_Part = (int)dat; //整数部分
  	Float_Part = (int32)((dat - (int)dat)*100000); //取小数位后5位
        OLED_Set_Pos(x,y); 
 	//整数部分显示
